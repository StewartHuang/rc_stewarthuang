...
type Supplier struct {
    ...
    RetryBaseDelayMs int64 `gorm:"not null;default:1000"`
    RetryMaxDelayMs int64  `gorm:"not null;default:240000"`
...
- 复合索引 `idx_status_next_retry` 通过 GORM 的 `index` 标签在 `Status` 和 `NextRetryAt` 字段上定义（SQLite 单索引支持多列）
